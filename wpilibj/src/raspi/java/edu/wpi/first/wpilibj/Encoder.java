...
/**
 * Class to read quadrature encoders. Quadrature encoders are devices that count
 * shaft rotation and can sense direction. The output of the Encoder class is an
 * integer that can count either up or down, and can go negative for reverse
 * direction counting. When creating Encoders, a direction can be supplied that
 * inverts the sense of the output to make code more readable if the encoder is
 ...
 * of phase with each other for direction sensing.
 *
 * <p>
...
public class Encoder extends SensorBase implements CounterBase, PIDSource,
	...
	public enum IndexingType {
		kResetWhileHigh(0), kResetWhileLow(1), kResetOnFallingEdge(2), kResetOnRisingEdge(
				3);
	...
	private void initEncoder(boolean reverseDirection, final EncodingType type) {
		throw new UnsupportedOperationException(
				"No current implemented encoder");
		/*
		 * m_encoder =
		 * EncoderJNI.initializeEncoder(m_aSource.getPortHandleForRouting(),
		 * m_aSource.getAnalogTriggerTypeForRouting(),
		 * m_bSource.getPortHandleForRouting(),
		 * m_bSource.getAnalogTriggerTypeForRouting(), reverseDirection,
		 * type.value);
		 * 
		 * m_pidSource = PIDSourceType.kDisplacement;
		 * 
		 * HAL.report(tResourceType.kResourceType_Encoder, getFPGAIndex(),
		 * type.value); LiveWindow.addSensor("Encoder", m_aSource.getChannel(),
		 * this);
		 */
	}
	 ...
	 *
	 * @param channelA
	 *            The a channel DIO channel. 0-9 are on-board, 10-25 are on the
	 *            MXP port
	 * @param channelB
	 *            The b channel DIO channel. 0-9 are on-board, 10-25 are on the
	 *            MXP port
	 * @param reverseDirection
	 ...
	 *            then a m_counter object will be used and the returned value
	 *            will either exactly match the spec'd count or be double (2x)
	 *            the spec'd count.
	 */
	public Encoder(final int channelA, final int channelB,
	 ...
	 *
	 * <p>
	 * <b>Warning:</b> This returns unscaled periods and getRate() scales using
	 * value from setDistancePerPulse().
	 *
	...
	/**
	 * Get the distance the robot has driven since the last reset as scaled by
	 * the value from {@link #setDistancePerPulse(double)}.
	 *
	...
	public void setIndexSource(int channel, IndexingType type) {
		if(m_allocatedI) {
			throw new AllocationException(
					"Digital Input for Indexing already allocated");
		}
	...
	public void setIndexSource(DigitalSource source, IndexingType type) {
		throw new UnsupportedOperationException(
				"No current implemented encoder");
		// EncoderJNI.setEncoderIndexSource(m_encoder,
	...
	public String getSmartDashboardType() {
		throw new UnsupportedOperationException(
				"No current implemented encoder");
		/*
		 * if(EncoderJNI.getEncoderEncodingType(m_encoder) ==
		 * EncodingType.k4X.value) { return "Quadrature Encoder"; } return
		 * "Encoder";
		 */
	}
	...
	public void updateTable() {
		throw new UnsupportedOperationException(
				"No current implemented encoder");
		/*
		 * if(m_table != null) { m_table.putNumber("Speed", getRate());
		 * m_table.putNumber("Distance", getDistance());
		 * m_table.putNumber("Distance per Tick",
		 * EncoderJNI.getEncoderDistancePerPulse(m_encoder)); }
		 */
	}
